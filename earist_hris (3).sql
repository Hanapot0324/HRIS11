...
(2, 'Ben', 'Michael', 'Santos', '2', '2013-02-14', 20134507, '2025-04-12 10:08:58', 2),
(3, 'Cara', 'Louise', 'Santos', NULL, '2011-12-03', 20134508, '2025-04-12 10:08:58', 3),
(4, 'Dylan', NULL, 'Santos', 'Jr.', '2016-09-22', 20134508, '2025-04-12 10:08:58', 4),
(5, 'Ella', 'Grace', 'Santos', NULL, '2018-05-09', 20134508, '2025-04-12 10:08:58', 5),
(6, 'Finn', NULL, 'Santos', NULL, '2020-07-15', 20134508, '2025-04-12 10:08:58', 6),
(7, 'Gina', 'Rose', 'Santos', NULL, '2017-03-30', 20134508, '2025-04-12 10:08:58', 7),
(8, 'Henry', NULL, 'Santos', NULL, '2019-11-18', 20134508, '2025-04-12 10:08:58', 8),
(9, 'Isla', 'Mae', 'Santos', NULL, '2021-01-05', 20134508, '2025-04-12 10:09:40', 9),
(10, 'Jack', 'Edward', 'Santos', NULL, '2014-04-17', 20134508, '2025-04-12 10:09:40', 10),
(11, 'Kyla', NULL, 'Santos', NULL, '2012-08-08', 20134508, '2025-04-12 10:09:40', 11),
(12, 'Leo', 'James', 'Santos', NULL, '2010-10-25', 20134508, '2025-04-12 10:09:40', 12),
(13, 'Mia', 'Isabelle', 'Santos', NULL, '2022-02-28', 20134508, '2025-04-12 10:09:40', 13),
(14, '', '', '', '', '', 0, '2025-04-12 21:00:05', 1),
(15, '', '', '', '', '', 0, '2025-04-12 21:00:05', 2);

--
-- Triggers `children_table`
--
DELIMITER $$
CREATE TRIGGER `before_insert_children` BEFORE INSERT ON `children_table` FOR EACH ROW BEGIN
  DECLARE nextIncValue INT;
  DECLARE maxIncValue INT;

  -- Find the smallest unused incValue for the same person_id
  SELECT MIN(t1.incValue + 1) INTO nextIncValue
  FROM children_table t1
  WHERE t1.person_id = NEW.person_id
    AND NOT EXISTS (
      SELECT 1 FROM children_table t2
      WHERE t2.person_id = t1.person_id
        AND t2.incValue = t1.incValue + 1
    );

  -- If incValue = 1 is not yet used
  IF NOT EXISTS (
      SELECT 1 FROM children_table
      WHERE person_id = NEW.person_id AND incValue = 1
  ) THEN
    SET NEW.incValue = 1;

  -- Else, assign the next smallest available incValue
  ELSEIF nextIncValue IS NOT NULL THEN
    SET NEW.incValue = nextIncValue;

  -- Else, assign the next number after the current max
  ELSE
    SELECT IFNULL(MAX(incValue), 0) + 1 INTO maxIncValue
    FROM children_table
    WHERE person_id = NEW.person_id;

    SET NEW.incValue = maxIncValue;
  END IF;
END
$$
DELIMITER ;
...
(33, 'EARIST', 'COLLEGE', '2020', '2026', 'N/A', '2026', 'N/A', 201508, '2025-04-04 06:21:45'),
...
INSERT INTO `department_assignment` (`id`, `code`, `name`, `employeeNumber`, `created_at`) VALUES
(1, 'CCS', 'Dhani San Jose', '20134507', '2025-03-29 04:16:48'),
(2, 'CAS', 'Trixie Morales', '123456', '2025-04-26 13:47:23');
...
(1, 'CCS', 'College of Computing Studies', '2025-03-29 04:17:23'),
(2, 'CAS', 'College of Art and Science', '2025-04-26 13:13:35'),
(3, 'BPA', 'Bachelor in Public Administration', '2025-04-26 16:48:35');
...
INSERT INTO `eligibility_table` (`id`, `eligibilityName`, `eligibilityRating`, `eligibilityDateOfExam`, `eligibilityPlaceOfExam`, `licenseNumber`, `DateOfValidity`, `person_id`, `incValue`, `created_at`) VALUES
(2, 'BAR Exam', 90.75, '2020-11-20', 'Quezon City', 'LIC002', '2025-11-20', 20134508, 2, '2025-04-11 16:51:34'),
(3, 'LET (Elementary)', 85.00, '2018-08-08', 'Cebu', 'LIC003', '2023-08-08', 20134508, 3, '2025-04-11 16:51:34'),
(4, 'NAPOLCOM', 82.50, '2021-01-15', 'Davao', 'LIC004', '2026-01-15', 20134508, 4, '2025-04-11 16:51:34'),
(5, 'CSC Sub-Professional', 87.00, '2022-02-25', 'Baguio', 'LIC005', '2027-02-25', 20134508, 5, '2025-04-11 16:51:34'),
(6, 'Foreign Service Officer Exam', 91.30, '2023-03-10', 'Manila', 'LIC006', '2028-03-10', 20134508, 6, '2025-04-11 16:51:34'),
(8, 'Civil Service Professional', 88.25, '2019-05-12', 'Manila', 'LIC001', '2024-05-12', 20134508, 1, '2025-04-11 16:55:00');

--
-- Triggers `eligibility_table`
--
DELIMITER $$
CREATE TRIGGER `before_insert_eligibility` BEFORE INSERT ON `eligibility_table` FOR EACH ROW BEGIN
  DECLARE nextIncValue INT;

  -- Find the smallest unused incValue for the same person_id
  SELECT MIN(t1.incValue + 1) INTO nextIncValue
  FROM eligibility_table t1
  WHERE t1.person_id = NEW.person_id
    AND NOT EXISTS (
      SELECT 1 FROM eligibility_table t2
      WHERE t2.person_id = t1.person_id
        AND t2.incValue = t1.incValue + 1
    );
  ...
  -- If incValue = 1 is not yet used
  IF NOT EXISTS (
      SELECT 1 FROM eligibility_table
      WHERE person_id = NEW.person_id AND incValue = 1
  ) THEN
    SET NEW.incValue = 1;
  ...
  -- If no gaps found and only one record exists, set incValue = 1
  ELSE
    SET NEW.incValue = 1;
  END IF;
END
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `before_insert_eligibility_table` BEFORE INSERT ON `eligibility_table` FOR EACH ROW BEGIN
  DECLARE maxInc INT;

  SELECT IFNULL(MAX(incValue), 0) INTO maxInc
  FROM eligibility_table
  WHERE person_id = NEW.person_id;

  IF maxInc >= 7 THEN
    SIGNAL SQLSTATE '45000'
    SET MESSAGE_TEXT = 'Each person_id can only have up to 7 eligibility records.';
  END IF;

  SET NEW.incValue = maxInc + 1;
END
$$
DELIMITER ;
...
--
-- Table structure for table `employee_salary_grade`
--

CREATE TABLE `employee_salary_grade` (
  `id` int(11) NOT NULL,
  `employeeNumber` int(11) NOT NULL,
  `plantilla_id` varchar(11) DEFAULT NULL,
  `step` varchar(11) NOT NULL,
  `created_at` timestamp NOT NULL DEFAULT current_timestamp()
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Dumping data for table `employee_salary_grade`
--

INSERT INTO `employee_salary_grade` (`id`, `employeeNumber`, `plantilla_id`, `step`, `created_at`) VALUES
(1, 20134507, '1', 'step5', '2025-04-20 23:47:31');
...
(1, '20134507', '2024-10-01', '2024-10-31', '00:00:00', '00:00:00', '00:00:00', '00:00:00', '06:45:19', '26:44:41', '01:43:11', '19:16:49', '06:28:04', '26:31:56', '32:16:54', '59:43:06');
...
CREATE TABLE `payroll` (
  ...
  `personalLifeRetIns` varchar(255) DEFAULT NULL,
...
INSERT INTO `payroll` (`id`, `employeeNumber`, `department`, `startDate`, `endDate`, `name`, `rateNbc188`, `grossSalary`, `abs`, `h`, `m`, `s`, `netSalary`, `withholdingTax`, `personalLifeRetIns`, `totalGsisDeds`, `totalPagibigDeds`, `philhealth`, `totalOtherDeds`, `totalDeductions`, `pay1st`, `pay2nd`, `rtIns`, `ec`, `dateCreated`) VALUES
(1, 20134507, 'CCS', '2024-10-01', '2024-10-31', 'Sarabia, Hanna N.', '15', NULL, NULL, '59', '43', '06', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, '2025-04-26 01:45:25');
...
CREATE TABLE `person_table` (
  ...
  `nameExtension` varchar(55) DEFAULT NULL,
  `placeOfBirth` varchar(20) DEFAULT NULL,
  `sex` varchar(20) DEFAULT NULL,
  `birthDate` varchar(20) DEFAULT NULL,
  `citizenship` varchar(11) DEFAULT NULL,
  `citizenship1` int(20) DEFAULT NULL,
  `civilStatus` varchar(20) DEFAULT NULL,
  `heightCm` varchar(10) DEFAULT NULL,
  `weightKg` varchar(10) DEFAULT NULL,
  `bloodType` varchar(3) DEFAULT NULL,
  `permanent_houseBlockLotNum` varchar(50) DEFAULT NULL,
  `permanent_streetName` varchar(50) DEFAULT NULL,
  `permanent_subdivisionOrVillage` varchar(50) DEFAULT NULL,
  `permanent_barangay` varchar(50) DEFAULT NULL,
  `permanent_cityOrMunicipality` varchar(50) DEFAULT NULL,
  `permanent_provinceName` varchar(50) DEFAULT NULL,
  ...
  `residential_houseBlockLotNum` varchar(50) DEFAULT NULL,
  `residential_streetName` varchar(100) DEFAULT NULL,
  `residential_subdivisionOrVillage` varchar(100) DEFAULT NULL,
  `residential_barangayName` varchar(100) DEFAULT NULL,
  `residential_cityOrMunicipality` varchar(100) DEFAULT NULL,
  `residential_provinceName` varchar(100) DEFAULT NULL,
...
INSERT INTO `person_table` (`id`, `firstName`, `middleName`, `lastName`, `nameExtension`, `placeOfBirth`, `sex`, `birthDate`, `citizenship`, `citizenship1`, `civilStatus`, `heightCm`, `weightKg`, `bloodType`, `permanent_houseBlockLotNum`, `permanent_streetName`, `permanent_subdivisionOrVillage`, `permanent_barangay`, `permanent_cityOrMunicipality`, `permanent_provinceName`, `permanent_zipcode`, `gsisNum`, `pagibigNum`, `philhealthNum`, `pZipCode`, `sssNum`, `tinNum`, `agencyEmployeeNum`, `residential_houseBlockLotNum`, `residential_streetName`, `residential_subdivisionOrVillage`, `residential_barangayName`, `residential_cityOrMunicipality`, `residential_provinceName`, `residential_zipcode`, `telephone`, `mobileNum`, `emailAddress`, `spouseFirstName`, `spouseMiddleName`, `spouseLastName`, `spouseNameExtension`, `spouseOccupation`, `spouseEmployerBusinessName`, `spouseBusinessAddress`, `spouseTelephone`, `fatherFirstName`, `fatherMiddleName`, `fatherLastName`, `fatherNameExtension`, `motherMaidenFirstName`, `motherMaidenMiddleName`, `motherMaidenLastName`, `elementaryNameOfSchool`, `elementaryDegree`, `elementaryPeriodFrom`, `elementaryPeriodTo`, `elementaryHighestAttained`, `elementaryYearGraduated`, `elementaryScholarshipAcademicHonorsReceived`, `secondaryNameOfSchool`, `secondaryDegree`, `secondaryPeriodFrom`, `secondaryPeriodTo`, `secondaryHighestAttained`, `secondaryYearGraduated`, `secondaryScholarshipAcademicHonorsReceived`) VALUES
(1, 'HANNA', 'N.', 'SARABIA', 'N/A', 'CALOOCAN', 'FEMALE', '2004/06/01', 'FILIPINO', NULL, 'SINGLE', '151', '50', 'O+', '123 ', 'GENERAL TIRONA ST', 'N/A', 'BRGY 145', 'CALOOCAN CITY', 'METRO MANILA', '1401', '123', '124', '125', NULL, '1226', '127', '20134507', '245', 'G. DE JESUS', 'N/A', 'BRGY 148', 'CALOOCAN', 'METRO MANILA', '1401', 'N/A', '09663122562', 'hannasarabia879@gmail.com', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'DELFIN', 'MIRANO', 'SARABIA', 'N/A', 'EDITA', 'LABAYNA', 'NARZOLES', 'BBES', 'ELEM GRAD', '2010', '2016', 'N/A', '2016', 'N/A', 'BBNHS', 'HIGH SCHOOL', '2016', '2020', NULL, '2020', 'hehe'),
(2, 'TRIXIE', 'ANN', 'MORALES', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, '16566', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);

-- --------------------------------------------------------

--
-- Table structure for table `plantilla_table`
--

CREATE TABLE `plantilla_table` (
  `id` int(11) NOT NULL,
  `employeeNumber` int(11) DEFAULT NULL,
  `plantilla_code` varchar(255) DEFAULT NULL,
  `position` varchar(255) NOT NULL,
  `salary_grade_id` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Dumping data for table `plantilla_table`
--

INSERT INTO `plantilla_table` (`id`, `employeeNumber`, `plantilla_code`, `position`, `salary_grade_id`) VALUES
(1, 20134507, '2', 'Instructor 1', 1);
...
--
-- Table structure for table `remittance_table`
--

CREATE TABLE `remittance_table` (
  `id` int(11) NOT NULL,
  `employeeNumber` int(11) NOT NULL,
  `disallowance` varchar(255) DEFAULT NULL,
  `gsisSalaryLoan` varchar(255) DEFAULT NULL,
  `gsisPolicyLoan` varchar(255) DEFAULT NULL,
  `gfal` varchar(255) DEFAULT NULL,
  `cpl` varchar(255) DEFAULT NULL,
  `mpl` varchar(255) DEFAULT NULL,
  `mplLite` varchar(255) DEFAULT NULL,
  `emergencyLoan` varchar(255) DEFAULT NULL,
  `nbc594` varchar(255) DEFAULT NULL,
  `increment` varchar(255) DEFAULT NULL,
  `pagibigFundCont` varchar(255) DEFAULT NULL,
  `pagibig2` varchar(255) DEFAULT NULL,
  `multiPurpLoan` varchar(255) DEFAULT NULL,
  `landbankSalaryLoan` varchar(255) DEFAULT NULL,
  `earistCreditCoop` varchar(255) DEFAULT NULL,
  `feu` text DEFAULT NULL,
  `created_at` timestamp NOT NULL DEFAULT current_timestamp()
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Dumping data for table `remittance_table`
--

INSERT INTO `remittance_table` (`id`, `employeeNumber`, `disallowance`, `gsisSalaryLoan`, `gsisPolicyLoan`, `gfal`, `cpl`, `mpl`, `mplLite`, `emergencyLoan`, `nbc594`, `increment`, `pagibigFundCont`, `pagibig2`, `multiPurpLoan`, `landbankSalaryLoan`, `earistCreditCoop`, `feu`, `created_at`) VALUES
(5, 20134507, '100', '200', '300', '400', '500', '600', '700', '800', '900', '1000', '1100', '1200', '1300', '1400', '1500', '1600', '2025-04-21 17:28:53');
...
(1, '2025', '1', '14061', '14164', '14278', '14393', '14509', '14626', '14743', '14862', '2025-04-21 15:28:43'),
(2, '2025', '2', '14925', '15035', '15146', '15258', '15371', '15484', '15599', '15714', '2025-04-21 15:28:43'),
(3, '2025', '3', '15852', '15971', '16088', '16208', '16329', '16448', '16571', '16693', '2025-04-21 15:28:43'),
(4, '2025', '4', '16833', '16958', '17084', '17209', '17337', '17464', '17594', '17724', '2025-04-21 15:28:43'),
(5, '2025', '5', '17866', '18000', '18133', '18267', '18401', '18538', '18676', '18813', '2025-04-21 15:28:43'),
(6, '2025', '6', '18957', '19098', '19239', '19383', '19526', '19670', '19816', '19963', '2025-04-21 15:28:43'),
(7, '2025', '7', '20110', '20258', '20408', '20560', '20711', '20865', '21019', '21175', '2025-04-21 15:28:43'),
(8, '2025', '8', '21448', '21642', '21839', '22035', '22234', '22435', '22638', '22843', '2025-04-21 15:28:43'),
(9, '2025', '9', '23226', '23411', '23599', '23788', '23978', '24170', '24364', '24558', '2025-04-21 15:28:43'),
(10, '2025', '10', '25586', '25790', '25996', '26203', '26412', '26623', '26835', '27050', '2025-04-21 15:28:43'),
(11, '2025', '11', '30024', '30308', '30597', '30889', '31185', '31486', '31790', '32099', '2025-04-21 15:28:44'),
(12, '2025', '12', '32245', '32529', '32817', '33108', '33403', '33702', '34004', '34310', '2025-04-21 15:28:44'),
(13, '2025', '13', '34421', '34733', '35049', '35369', '35694', '36022', '36354', '36691', '2025-04-21 15:28:44'),
(14, '2025', '14', '37024', '37384', '37749', '38118', '38491', '38869', '39252', '39640', '2025-04-21 15:28:44'),
(15, '2025', '15', '40208', '40604', '41006', '41413', '41824', '42241', '42662', '43090', '2025-04-21 15:28:44'),
(16, '2025', '16', '43560', '43996', '44438', '44885', '45338', '45796', '46261', '46730', '2025-04-21 15:28:44'),
(17, '2025', '17', '47247', '47727', '48213', '48705', '49203', '49708', '50218', '50735', '2025-04-21 15:28:44'),
(18, '2025', '18', '51304', '51832', '52367', '52907', '53456', '54010', '54572', '55140', '2025-04-21 15:28:44'),
(19, '2025', '19', '56390', '57165', '57953', '58753', '59567', '60394', '61235', '62089', '2025-04-21 15:28:45'),
(20, '2025', '20', '62967', '63842', '64732', '65637', '66557', '67479', '68409', '69342', '2025-04-21 15:28:45'),
(21, '2025', '21', '70013', '71000', '72004', '73024', '74061', '75115', '76151', '77239', '2025-04-21 15:28:45'),
(22, '2025', '22', '78162', '79277', '80411', '81564', '82735', '83887', '85096', '86324', '2025-04-21 15:28:45'),
(23, '2025', '23', '87315', '88574', '89855', '91163', '92592', '94043', '95518', '96955', '2025-04-21 15:28:45'),
(24, '2025', '24', '98185', '99721', '101283', '102871', '104483', '106123', '107739', '109431', '2025-04-21 15:28:45'),
(25, '2025', '25', '111727', '113476', '115254', '117062', '118899', '120766', '122664', '124591', '2025-04-21 15:28:46'),
(26, '2025', '26', '126252', '128228', '130238', '132280', '134356', '136465', '138608', '140788', '2025-04-21 15:28:46'),
(27, '2025', '27', '142663', '144897', '147169', '149407', '151752', '153850', '156267', '158723', '2025-04-21 15:28:46'),
(28, '2025', '28', '160469', '162988', '165548', '167994', '170634', '173320', '175803', '178572', '2025-04-21 15:28:46'),
(29, '2025', '29', '180492', '183332', '186218', '189151', '192131', '194797', '197870', '200993', '2025-04-21 15:28:46'),
(30, '2025', '30', '203200', '206401', '209558', '212766', '216022', '219434', '222797', '226319', '2025-04-21 15:28:46'),
(31, '2025', '31', '293191', '298773', '304464', '310119', '315883', '321846', '327895', '334059', '2025-04-21 15:28:46'),
(32, '2025', '32', '347888', '354743', '361736', '368694', '375969', '383391', '390963', '398686', '2025-04-21 15:28:46'),
(33, '2025', '33', '438844', '451713', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', 'N/A', '2025-04-21 15:28:46');
...
(22, 'BBSHS', 'SENIOR HIGH', '2020', '2022', 'N/A', '2022', 'N/A', 20134508, '2025-04-04 17:44:31');
...
INSERT INTO `voluntary_work_table` (`id`, `nameAndAddress`, `dateFrom`, `dateTo`, `numberOfHours`, `natureOfWork`, `person_id`, `created_at`, `incValue`) VALUES
(2, 'Gawad Kalinga - Quezon City', '2021-07-01', '2021-07-10', 30, 'Community Building', 20134508, '2025-04-12 11:39:09', 2),
(3, 'Tree Planting - Antipolo', '2023-03-20', '2023-03-21', 8, 'Environmental Effort', 20134508, '2025-04-12 11:39:09', 3),
(4, 'Feeding Program - Tondo', '2020-12-05', '2020-12-05', 6, 'Feeding Children', 20134508, '2025-04-12 11:39:09', 4),
(5, 'Brigada Eskwela - Pasig', '2021-05-10', '2021-05-12', 12, 'School Clean-Up', 20134508, '2025-04-12 11:39:09', 5),
(6, 'Disaster Relief - Marikina', '2022-11-15', '2022-11-20', 25, 'Disaster Response', 20134508, '2025-04-12 11:39:09', 6),
(7, 'Cleanup Drive - Manila Bay', '2023-02-10', '2023-02-10', 5, 'Coastal Cleanup', 20134508, '2025-04-12 11:39:09', 7),
(8, 'Senior Home Visit - Mandaluyong', '2020-08-01', '2020-08-03', 10, 'Companionship and Aid', 20134508, '2025-04-12 11:39:09', 8),
(9, 'Angat Buhay', '2025-04-12', '2027-04-12', 40, 'NGO', 20134508, '2025-04-12 11:40:05', 1);

--
-- Triggers `voluntary_work_table`
--
DELIMITER $$
CREATE TRIGGER `before_insert_voluntary_work` BEFORE INSERT ON `voluntary_work_table` FOR EACH ROW BEGIN
  DECLARE nextIncValue INT;
  DECLARE maxIncValue INT;

  -- Find the smallest unused incValue for the same person_id
  SELECT MIN(t1.incValue + 1) INTO nextIncValue
  FROM voluntary_work_table t1
  WHERE t1.person_id = NEW.person_id
    AND NOT EXISTS (
      SELECT 1 FROM voluntary_work_table t2
      WHERE t2.person_id = t1.person_id
        AND t2.incValue = t1.incValue + 1
    );
  ...
  -- If incValue = 1 is not yet used
  IF NOT EXISTS (
      SELECT 1 FROM voluntary_work_table
      WHERE person_id = NEW.person_id AND incValue = 1
  ) THEN
    SET NEW.incValue = 1;
...
CREATE TABLE `work_experience_table` (
  ...
  `person_id` int(11) DEFAULT NULL,
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
  `incValue` int(11) NOT NULL DEFAULT 1
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
INSERT INTO `work_experience_table` (`id`, `workDateFrom`, `workDateTo`, `workPositionTitle`, `workCompany`, `workMonthlySalary`, `SalaryJobOrPayGrade`, `StatusOfAppointment`, `isGovtService`, `person_id`, `created_at`, `incValue`) VALUES
(3, '2018-01', '2018-12', 'Senior Developer', 'NextGen Solutions', 40000.00, 'SG-15', 'Permanent', 'No', 20134508, '2025-04-11 16:29:55', 3),
(4, '2019-01', '2020-01', 'Lead Developer', 'NextGen Solutions', 45000.00, 'SG-18', 'Permanent', 'No', 20134508, '2025-04-11 16:29:55', 4),
(5, '2020-02', '2021-06', 'Project Manager', 'NextGen Solutions', 50000.00, 'SG-20', 'Permanent', 'No', 20134508, '2025-04-11 16:29:55', 5),
(6, '2021-07', '2022-07', 'IT Consultant', 'SmartTech PH', 55000.00, 'SG-22', 'Consultant', 'No', 20134508, '2025-04-11 16:29:55', 6),
(7, '2013-05', '2014-12', 'Tech Support', 'TechnoHelp', 22000.00, 'SG-9', 'Contractual', 'No', 20134508, '2025-04-11 16:29:55', 7),
(8, '2012-03', '2013-04', 'Intern', 'EduSoft', 15000.00, 'SG-5', 'Internship', 'No', 20134508, '2025-04-11 16:29:55', 8),
(9, '2022-08', '2023-06', 'IT Supervisor', 'Metro IT Hub', 60000.00, 'SG-23', 'Permanent', 'Yes', 20134508, '2025-04-11 16:29:55', 9),
(10, '2023-07', '2024-01', 'Systems Analyst', 'GovData Center', 62000.00, 'SG-24', 'Permanent', 'Yes', 20134508, '2025-04-11 16:29:55', 10),
(11, '2011-01', '2011-12', 'Assistant', 'Municipality Office', 18000.00, 'SG-6', 'Contractual', 'Yes', 20134508, '2025-04-11 16:29:55', 11),
(12, '2010-06', '2010-12', 'Volunteer', 'Community Center', 0.00, 'N/A', 'Volunteer', 'Yes', 20134508, '2025-04-11 16:29:55', 12),
(13, '2024-02', '2024-12', 'Database Admin', 'CloudBase Ltd.', 65000.00, 'SG-25', 'Permanent', 'No', 20134508, '2025-04-11 16:29:55', 13),
(14, '2025-01', '2025-04', 'Tech Lead', 'CyberBridge', 70000.00, 'SG-26', 'Probationary', 'No', 20134508, '2025-04-11 16:29:55', 14),
(15, '2009-02', '2009-12', 'Library Assistant', 'City Library', 12000.00, 'SG-4', 'Part-time', 'Yes', 20134508, '2025-04-11 16:29:55', 15),
(16, '2024-01', '2024-04', 'Scrum Master', 'AgileWorks PH', 68000.00, 'SG-24', 'Consultant', 'No', 20134508, '2025-04-11 16:29:55', 16),
(17, '2019-06', '2019-12', 'UX Designer', 'Creative Pixel', 39000.00, 'SG-14', 'Project-based', 'No', 20134508, '2025-04-11 16:29:55', 17),
(18, '2017-03', '2018-03', 'QA Analyst', 'SoftGuard Ltd.', 31000.00, 'SG-12', 'Contractual', 'No', 20134508, '2025-04-11 16:29:55', 18),
(19, '2023-03', '2023-10', 'IT Auditor', 'SecureGov PH', 64000.00, 'SG-23', 'Permanent', 'Yes', 20134508, '2025-04-11 16:29:55', 19),
(20, '2014-01', '2014-11', 'Junior Tech Officer', 'Public Works Dept.', 20000.00, 'SG-10', 'Temporary', 'Yes', 20134508, '2025-04-11 16:29:55', 20),
(21, '2016-01', '2016-06', 'R&D Assistant', 'SmartSoft', 28000.00, 'SG-11', 'Project-based', 'No', 20134508, '2025-04-11 16:29:55', 21),
(22, '2012-01', '2012-12', 'Student Assistant', 'University Registrar', 10000.00, 'SG-3', 'Student', 'Yes', 20134508, '2025-04-11 16:29:55', 22),
(23, '2015-07', '2015-12', 'Junior QA', 'BugSquashers Inc.', 23000.00, 'SG-10', 'Contractual', 'No', 20134508, '2025-04-11 16:29:55', 23),
(24, '2020-07', '2020-12', 'Business Analyst', 'DataLink', 46000.00, 'SG-19', 'Probationary', 'No', 20134508, '2025-04-11 16:29:55', 24),
(25, '2021-01', '2021-06', 'Network Admin', 'TelNet PH', 48000.00, 'SG-21', 'Permanent', 'No', 20134508, '2025-04-11 16:29:55', 25),
(26, '2018-04', '2019-03', 'Tech Trainer', 'EduIT Solutions', 37000.00, 'SG-13', 'Contractual', 'No', 20134508, '2025-04-11 16:29:55', 26),
(27, '2025/04/11', '2027/04/11', 'HEHE', 'HEHE', 90000.00, 'SJ', '', '', 20134508, '2025-04-11 16:40:48', 1),
(28, '2024', '2026', 'HIHI', 'EARIST', 32000.00, 'PG 4', '', '', 20134508, '2025-04-12 10:25:19', 2);

--
-- Triggers `work_experience_table`
--
DELIMITER $$
CREATE TRIGGER `before_insert_work_experience` BEFORE INSERT ON `work_experience_table` FOR EACH ROW BEGIN
  DECLARE nextIncValue INT;
  DECLARE maxIncValue INT;

  -- Find the smallest unused incValue for the same person_id
  SELECT MIN(t1.incValue + 1) INTO nextIncValue
  FROM work_experience_table t1
  WHERE t1.person_id = NEW.person_id
    AND NOT EXISTS (
      SELECT 1 FROM work_experience_table t2
      WHERE t2.person_id = t1.person_id
        AND t2.incValue = t1.incValue + 1
    );
  ...
  -- If incValue = 1 is not yet used
  IF NOT EXISTS (
      SELECT 1 FROM work_experience_table
      WHERE person_id = NEW.person_id AND incValue = 1
  ) THEN
    SET NEW.incValue = 1;
...
--
-- Indexes for table `employee_salary_grade`
--
ALTER TABLE `employee_salary_grade`
  ADD PRIMARY KEY (`id`);
...
ALTER TABLE `person_table`
  ADD PRIMARY KEY (`id`),
  ADD KEY `fk_agencyEmployeeNum` (`agencyEmployeeNum`);

--
-- Indexes for table `plantilla_table`
--
ALTER TABLE `plantilla_table`
  ADD PRIMARY KEY (`id`);

--
-- Indexes for table `remittance_table`
--
ALTER TABLE `remittance_table`
  ADD PRIMARY KEY (`id`);
...
--
-- AUTO_INCREMENT for table `employee_salary_grade`
--
ALTER TABLE `employee_salary_grade`
...
--
-- AUTO_INCREMENT for table `plantilla_table`
--
ALTER TABLE `plantilla_table`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;

--
-- AUTO_INCREMENT for table `remittance_table`
--
ALTER TABLE `remittance_table`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=6;
...
--
-- Constraints for table `person_table`
--
ALTER TABLE `person_table`
  ADD CONSTRAINT `fk_agencyEmployeeNum` FOREIGN KEY (`agencyEmployeeNum`) REFERENCES `users` (`employeeNumber`) ON DELETE CASCADE ON UPDATE CASCADE;
COMMIT;
